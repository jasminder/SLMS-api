...
export const createGroupHomeworkHandler = async (req: Request<{}, {}, CreateGroupHomeworkSchema['body'], {}>, res: Response, next: NextFunction) => {
    const { studentId, teacherId, title, className, roomName, sectionId, termSubjectLevelId, attachments, description } = req.body;
    const feedback = await createGroupHomework(studentId, teacherId, termSubjectLevelId, sectionId, title, description, attachments, className, roomName);
